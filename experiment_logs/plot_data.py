...
# Throughput
ax = axes[0, 0]
print("[INFO] Plotting Time-Series Throughput")

for name, folder in SCHEMES.items():
    base = os.path.join(BASE_ROOT, folder)
    log_path = os.path.join(base, THROUGHPUT_LOG)
    print("  → Loading %s from %s" % (name, log_path))
    t, thr = load_throughput(log_path)
    print("    → Data points: %d" % len(t))
    if t.size:
        ax.plot(t, thr, label=name)
    else:
        print("    [WARNING] No throughput data for %s" % name)

handles, labels = ax.get_legend_handles_labels()
if handles:
    ax.legend()
else:
    print("[WARNING] No valid throughput lines plotted!")
